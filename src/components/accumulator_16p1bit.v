...
//  wire cộng và check overflow phía trên
...
always @(posedge clk or posedge rst) begin
    if (rst) begin
        ...
        if (clear_mode) begin
            ...
            if (signed_mode) begin
                // Xử lý CLAMP cho SIGNED
                if (signed_overflow) begin
                    // Nếu overflow số dương (tổng ra âm): Clamp lên Max Positive
                    // Nếu overflow số âm (tổng ra dương): Clamp xuống Max Negative
                    if (signed_accumulator[15] == 1'b0) begin 
                        accumulator_value <= 17'h07FFF; // Max Pos
                        result_out <= 16'h7FFF;
                    end else begin
                        accumulator_value <= 17'h18000; // Max Neg (dạng 17-bit)
                        result_out <= 16'h8000;
                    end
                end else begin
                    accumulator_value <= signed_add_result;
                ...
                // Xử lý CLAMP cho UNSIGNED
                if (unsigned_overflow) begin
                    accumulator_value <= 17'h0FFFF; // Clamp tại 65535
                    result_out <= 16'hFFFF;
                end else begin
                    accumulator_value <= add_result;
                    result_out <= add_result[15:0];
